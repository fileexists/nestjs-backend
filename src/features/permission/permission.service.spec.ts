...
describe('PermissionService', () => {
  ...
  describe('create', () => {
    it('should create and persist a new permission entity', async () => {
      const dto: PermissionDTO = { name: 'MODERATOR', description: 'Can moderate content' };
      const created = buildPermission({ name: 'MODERATOR', description: dto.description });
    ...
    it('should propagate a duplicate-entry database error from repository.save', async () => {
      const dto: PermissionDTO = { name: 'USER' };
  ...
  describe('remove', () => {
    it('should call repository.delete with the permission id', async () => {
      ...
      expect(mockPermissionRepository.delete).toHaveBeenCalledWith('perm-uuid-1');
    });

    it('should resolve without error even when the id does not exist (silent delete)', async () => {
      mockPermissionRepository.delete.mockResolvedValue({ affected: 0 });

      await expect(service.remove('non-existent-id')).resolves.toBeUndefined();
    });
  });
});
